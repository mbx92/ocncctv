...
const { data: quote, error } = await useFetch(`/api/public/packages/${route.params.token}`)

useHead({
  title: computed(() => (quote.value ? quote.value.title || 'Paket' : 'Paket'))
})
...
async function downloadPdf() {
  ...
  pdfBusy.value = true
  try {
    const blob = await $fetch(`/api/public/packages/${route.params.token}/pdf`, { responseType: 'blob' })
    const href = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = href
    a.download = `${quote.value.quoteNumber || 'paket'}.pdf`
